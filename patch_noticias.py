path = r'C:\Users\eduar\OneDrive\Desktop\Antigravity\ArenaCopa\Arenacopa\src\components\copa\NoticiasTab.tsx'
    ...
    <div className="space-y-6 text-white">
        ...
        <div className="rounded-[28px] border border-primary/30 bg-primary/10 p-5">
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-primary">
              {t("noticias.priority_team")}
            </p>
          </div>
          <h2 className="text-xl font-black">{topPersonalized[0].title}</h2>
          <p className="mt-2 text-sm text-zinc-300">
            {topPersonalized[0].summary || t("noticias.priority_summary_fallback")}
          </p>
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((category) => (
          ...
          >
            {t(`noticias.categories.${category}`)}
          </button>
        ))}
      </div>
          ...
          <article key={item.id} className="rounded-[28px] border border-white/10 bg-white/5 p-5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-zinc-300">
                ...
                {t(`noticias.categories.${(item.category || "general").toLowerCase()}`, {
                  defaultValue: (item.category || "general").toUpperCase(),
                })}
              </div>
              {item.source_name && (
            ...
            <p className="mt-2 text-sm text-zinc-400">
              {item.summary || t("noticias.summary_fallback")}
            </p>

            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <div className="text-xs text-zinc-500">
                {item.published_at
                  ? new Date(item.published_at).toLocaleString(i18n.language)
                  : t("noticias.date_unknown")}
              </div>
...
print(f'NoticiasTab.tsx written ({new_content.count(chr(10))} lines)')
