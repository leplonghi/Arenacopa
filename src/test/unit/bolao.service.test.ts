...
vi.mock("@/services/boloes/bolao-config.service", () => ({
  leaveBolao: vi.fn(async () => undefined),
  updatePoolMemberPaymentStatus: vi.fn(async () => undefined),
}));
...
describe("bolao.service", () => {
  ...
  describe("removeBolaoMember", () => {
    it("remove o membro chamando leaveBolao", async () => {
      await removeBolaoMember("bolao-1", "user-123");

      expect(leaveBolao).toHaveBeenCalledOnce();
      expect(leaveBolao).toHaveBeenCalledWith({ payload: { bolao_id: "bolao-1" } });
    });

    it("lança AppError com código BOLAO_REMOVE_MEMBER_FAILED em caso de erro", async () => {
      vi.mocked(leaveBolao).mockRejectedValueOnce({ code: "permission-denied" });
  ...
  describe("updateBolaoMemberPaymentStatus", () => {
    it("atualiza o status de pagamento chamando updatePoolMemberPaymentStatus", async () => {
      ...
      expect(updatePoolMemberPaymentStatus).toHaveBeenCalledOnce();
      expect(updatePoolMemberPaymentStatus).toHaveBeenCalledWith({
        payload: {
          bolao_id: "bolao-1",
          member_id: "user-123_bolao-1",
          payment_status: "paid",
        },
      });
    });
    ...
    it("lança AppError com código BOLAO_UPDATE_PAYMENT_FAILED em caso de erro", async () => {
      vi.mocked(updatePoolMemberPaymentStatus).mockRejectedValueOnce(new Error("Update failed"));
    ...
    it("aceita diferentes status de pagamento (pending, paid, free)", async () => {
      for (const status of ["pending", "paid", "free"] as const) {
        vi.mocked(updatePoolMemberPaymentStatus).mockClear();
        await updateBolaoMemberPaymentStatus({
          bolaoId: "bolao-1",
          userId: "user-123",
