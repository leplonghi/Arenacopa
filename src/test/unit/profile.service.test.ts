...
describe("profile.service", () => {
  ...
  describe("ensureProfile", () => {
    it("cria perfil quando ainda não existe", async () => {
      ...
      const profileSetCall = mockSetDoc.mock.calls.find(call => call[0].path === "profiles/user-new");
      expect(profileSetCall).toBeDefined();
      expect(profileSetCall![1]).toMatchObject({ name: "Novo Usuário", user_id: "user-new" });
    });
    ...
    it("não cria perfil quando já existe", async () => {
      ...
      const profileSetCall = mockSetDoc.mock.calls.find(call => call[0].path === "profiles/user-123");
    ...
    it("usa e-mail como nome fallback quando sem metadata", async () => {
      ...
      const profileSetCall = mockSetDoc.mock.calls.find(call => call[0].path === "profiles/user-abc");
      expect(profileSetCall![1]).toMatchObject({ name: "torcedor" });
    });
