/**
 * seed_matches_correct.mjs
 * Seeds the correct Firestore project (arenacopa-web-2026) from .env
 * Uses the CLIENT SDK — no ADC / firebase-admin needed
 */
...
if (!config.projectId) { console.error("Missing projectId"); process.exit(1); }
...
console.log(`Total matches to seed: ${matches.length}`);
...
async function batchWrite(colName, docs) {
  ...
  for (let i = 0; i < docs.length; i += SIZE) {
    ...
    for (const item of docs.slice(i, i + SIZE)) {
      const { id, ...data } = item;
      batch.set(doc(collection(db, colName), id), data);
    }
    await batch.commit();
    written += docs.slice(i, i + SIZE).length;
    console.log(`  ${colName}: ${written}/${docs.length} written`);
  }
}

await batchWrite("matches", matches);
console.log(`\nDone! ${matches.length} matches seeded to ${config.projectId}`);
process.exit(0);
